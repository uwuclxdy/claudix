...
pub fn tool_definitions() -> Vec<Value> {
    vec![
        json!({
            "name": "search_code",
            "description": "Semantic code search over the active project (plus optional cross-repos). Use when: looking for code by meaning ('where is auth handled?', 'how does config load?'), looking up an identifier ('handle_session_start'), exploring an unfamiliar area, or checking whether logic already exists before implementing something new. Prefer over Grep for anything that isn't an exact literal or regex match. Args: query (required); optional top_k, language_filter, path_prefix, repos (extra indexed repos searched alongside the active project, which is always included). Returns: hits grouped by directory, ordered by best-hit score, each with repo, file path, line range, kind, name, snippet, and score; cross-repo runs add repo_errors for unindexed or model-mismatched repos. A hit marked stale means its file changed on disk after indexing: treat its line numbers as approximate and Read the file to confirm.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        json!({
            ...
            "description": "Find near-identical code chunks across files using stored embeddings. Use when: checking whether equivalent code already exists BEFORE adding new logic, or auditing for copy-paste within this repo or across an explicit list of already-indexed repos. Args: optional min_similarity (0-1, default 0.85; raise toward 1.0 for exact copies, lower for looser matches), limit (default 50), repos (absolute paths; when set, ONLY these repos are scanned and the active project is NOT auto-added, unlike search_code which always includes it). Returns: pairs sorted by similarity descending, each with repo, file path, line range, and name for both sides; plus repo_errors for any listed repo that is unindexed or model-mismatched.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "min_similarity": {
